...
*   **`HacerPedido`**:
    ...
    *   **Nota Importante:** No incluyas `{pedidoDetalle}` en las frases de ejemplo. El tipo de slot `AMAZON.FreeFormInput` está diseñado para capturar la respuesta del usuario *después* de que Lex haga la pregunta (el "prompt").
*   **`PreguntaAbierta`**:
